...
EMAIL_ADDRESS="yacoutamia@gmail.com"
...
function main(){
  ...
  SERVICES='sabnzbd sonarr radarr bazarr transmission'
  ...
  if [ $SCRUB_PERCENT -gt 0 ]; then
    ...
    if [ $CHK_FAIL -eq 1 -a $DO_SYNC -eq 0 ]; then
      ...
      if [ $DO_SYNC -eq 1 -a -z "$(grep -w "SYNC_JOB-" $TMP_OUTPUT)" ]; then
        ...
        curl -m 10 --retry 5 https://hc-ping.com/0ef0cd5f-0911-4815-9d3c-caa7a64a2754
      fi
    fi
  else
...
function output_to_file_screen(){
  ...
  exec {out}>&1 {err}>&2
  # NOTE: Not preferred format but valid: 
